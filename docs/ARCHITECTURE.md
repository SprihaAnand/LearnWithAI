...
LearnWithAI begins as a single Python application so an NGO or delivery partner can run a real pilot without managing a complex platform. The browser application and JSON API share one origin, while SQLite makes the first deployment portable and easy to back up. Local automatic transcription adds the focused `faster-whisper` runtime dependency, without making video transcription depend on a Gemini key or external AI service.
...
LearnWithAI Python process
  ...
  |-- background local faster-whisper transcription
  |     \-- model cache / CPU or optional GPU
  \-- Gemini API for session-keyed Q&A only

SQLite database          data/uploads/
data/learnwithai.db      private pilot media
...
- Local faster-whisper transcription for uploaded video when the administrator selects it.
- Gemini Q&A only when a learner has supplied a session-only key and selected a ready transcript.
...
A Gemini key is submitted over the authenticated same-origin session, held only in backend memory against the hash of that session token, and cleared on key removal, logout, token expiry, or server restart. The key is never written to SQLite, returned by an API, placed in a URL, or logged. It authorises Gemini Q&A only; automatic local transcription does not read, require, or send this key.
...
An upload can use an administrator-provided UTF-8 `.txt`, `.srt`, or `.vtt` transcript, or start a local faster-whisper background transcription job. The service tracks `queued`, `processing`, `ready`, and `failed` states. Automatically generated text is recorded with `transcript_source` set to `whisper` and is explicitly a review-required draft, rather than an authoritative verbatim record. The uploaded media remains local to the deployment during automatic transcription; it is not sent to Gemini.
...
The tutor is designed to support learning, not replace programme staff or make high-stakes decisions. A learner's Gemini key is an explicit session-only choice, and all Gemini calls occur through the backend. When a ready transcript is selected, Gemini receives only the bounded lesson transcript and learner question. The prompt requires it to answer only from that material, acknowledge unsupported questions, and avoid professional medical, legal, or safety advice. Without a session key or a ready transcript, the app directs the learner to connect Gemini or select a transcript-ready lesson rather than sending an ungrounded question.

Automatic transcription is a separate local job. After an administrator chooses it, faster-whisper reads the uploaded video in the deployment environment and produces a `whisper`-sourced draft. It needs no Gemini key and does not send the video to Gemini. The runtime needs the `faster-whisper` dependency, a writable model cache, and model weights; PyAV packages the FFmpeg libraries needed for decoding, so a separate system FFmpeg binary is not a prerequisite. `LEARNWITHAI_WHISPER_MODEL=base`, `LEARNWITHAI_WHISPER_DEVICE=cpu`, and `LEARNWITHAI_WHISPER_COMPUTE_TYPE=int8` are safe defaults; operators can select a supported model/device/compute combination for their hardware. CPU execution is supported, while a compatible CUDA/CTranslate2 GPU runtime is optional for throughput. See [the Gemini and video workflow](GEMINI_VIDEO_WORKFLOW.md) for operator-facing details and the mandatory human-review step.
...
Run one server process behind a TLS-terminating reverse proxy. Bind the Python process to a private interface, configure environment variables through the hosting platform, place SQLite on durable storage, and remove or reset the development demo accounts. Install `requirements.txt`, provision a writable faster-whisper model cache, and pre-warm the selected model before accepting real uploads. CPU is sufficient for a modest pilot; test optional GPU acceleration with the exact deployment image rather than relying on host detection.
...
Move persistent data to a managed relational service before running multiple API instances. Use object storage and a CDN/video host for content delivery, move local faster-whisper jobs into isolated workers or a queue before they compete with learner traffic, and establish an analytics pipeline with privacy and retention controls.

## Verification

Install runtime dependencies, then run the repository's baseline verification:
...
GitHub Actions installs the pinned runtime requirements, then runs these checks on supported Python versions.
